...
export default function Reports() {
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((o) => {
      const ts = new Date(o.orderTime).getTime();
      return ts >= f && ts <= t;
    });
  }, [orders, from, to]);
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionPanel
        title="Revenue trend"
        subtitle="Last 7 days vs your filter range."
      >
      ...
      <SectionPanel
        title="Order history"
        subtitle={`Showing ${filtered.length} orders · ${completed} completed`}
        noPadding
      >
            ...
            <tbody>
              {filtered.map((o) => (
                <tr
                  ...
                  data-testid={`row-report-${o.orderId}`}
                >
                  <td className="py-3.5 pl-5 pr-3 font-medium">{o.orderId}</td>
                  <td className="py-3.5 px-3">{o.userDetails.name}</td>
                  ...
                  <td className="py-3.5 pr-5 text-muted-foreground">
                    {dateShort(o.orderTime.toString())}
                  </td>
                </tr>
              ))}
            </tbody>
